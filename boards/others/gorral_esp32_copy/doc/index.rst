.. zephyr:board:: gorral_esp32_copy
...
process. For more information, check `Gorral ESP32 Copy`_.
...
Gorral ESP32 Copy-WROVER allows 2 different applications to be executed in ESP32 SoC. Due to its dual-core architecture, each core can be enabled to execute customized tasks in stand-alone mode
...
.. zephyr-app-commands::
   :zephyr-app: samples/hello_world
   :board: gorral_esp32_copy/esp32/procpu
   :goals: build

The usual ``flash`` target will work with the ``gorral_esp32_copy`` board
...
On the Gorral ESP32 Copy board, the JTAG pins are not run to a
...
.. _`Gorral ESP32 Copy`: https://docs.espressif.com/projects/esp-dev-kits/en/latest/esp32/Gorral ESP32 Copy/index.html
